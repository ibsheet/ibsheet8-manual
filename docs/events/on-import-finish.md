...
<!-- synonyms: 업로드 완료 이벤트, 엑셀 로드 완료, import finish, 로드 중단 -->

> 엑셀/텍스트 로드 시 호출되는 이벤트입니다.  
...
| result | `number` | **로딩 결과 코드**<br>`0`:성공<br>`음수`:로딩 에러 발생 |
| message | `string` | **로딩 결과 메세지**<br>`""`:성공<br>`그외`:에러 메세지 |

loadExcel시 에러가 발생했을 경우, 각 음수 코드에 대응하는 에러는 다음과 같습니다.  
