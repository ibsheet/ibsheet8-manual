...
|prefix|`string`|`Shift, Alt, Ctrl` 키와 같이 눌려진 경우 key 이름<br>`Shift-> Ctrl-> Alt` 순으로 문자열을 조합하여 리턴.<br>가령 `Shift`와 `Ctrl`이 같이 눌려진 경우 `"ShiftCtrl"` 이 리턴, `Alt`와 `Ctrl`이 같이 눌려진 경우 `"CtrlAlt"`가 리턴|
...
- [키보드 입력 이벤트 발생 순서](/docs/events/04-key-event-flow)
