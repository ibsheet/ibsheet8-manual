...
>
> 해당 이벤트가 호출되는 시점에서는 아직 값이 수정되기 이전의 값이 얻어짐으로, 위와 같은 타입의 열에서는 가급적 [onAfterChange](./on-after-change) 나 [onAfterClick](./on-after-click) 이벤트에서 클릭에 대한 결과를 처리하실 것을 권합니다.
...
- [마우스 클릭 이벤트 발생 순서](/docs/events/03-click-event-flow)
