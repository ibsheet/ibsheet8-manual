...
|-----|-----|-----|
|mode|`number`|`0`: `Enter` 키를 누르면 `값 편집(edit) 시작`, (편집 상태) `Enter` 키를 누르면 `편집 종료` (`default`) <br/>`1`: `아래 셀(down)로 이동`, 마지막 셀에서 첫 행, 첫 열으로 이동 <br/>`2`: `아래 셀(down)로 이동`, 각 열의 마지막 셀에서 행 추가 후 이동 <br/>`3`: `오른쪽 셀(tab)로 이동`, 마지막 셀에서 첫 행, 첫 열으로 이동 <br/>`4`: `오른쪽 셀(tab)로 이동`, 마지막 셀에서 행 추가 후 첫 열으로 이동 <br/>`5`: `아래 셀(down)로 이동`, 이동 중 해당 컬럼의 마지막 셀인 경우 다음 열로 이동. 이동할 수 있는 가장 마지막 열의 마지막 셀에서 포커스 유지 <br/>`6`: `오른쪽 셀(tab)로 이동`, 이동 중 해당 행의 마지막 셀인 경우 다음 행으로 이동. 이동할 수 있는 가장 마지막 행의 마지막 셀에서 포커스 유지 <br/>`8`: `아래 셀(down)로 이동`, 시작 컬럼의 이동할 수 있는 최하단 셀에서 포커스 유지 <br/>`9`: `오른쪽 셀(tab)로 이동`, 시작 행의 이동할 수 있는 최우측 셀에서 포커스 유지|
...
|core|8.4.0.19|`mode: 8`, `mode: 9` 추가|
<!--!
