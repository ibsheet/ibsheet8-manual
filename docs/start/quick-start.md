...
<!-- synonyms: 시작하기, 빠른 시작, 설치, 셋업, setup, getting started, 시트 생성, 첫 시트, 튜토리얼, 따라하기, 스크립트 로드, 파일 로드, import, ibsheet.js, ko.js, locale, 로케일, main.css, css 로드, ibleaders.js, 라이선스 파일, 로드 순서, 예제, 샘플 코드, IBSheet 사용법, 처음 만들기, 시트 안 뜸, 시트 안 보임, hello world -->
...
IBSheet8은 서버에 설치해 동작하는 제품이 아니라, 웹 화면 개발에 사용하는 **자바스크립트 라이브러리**입니다.  
...
- **위치**: 시트를 생성하거나 사용하는 스크립트보다 **먼저 로드**되도록 추가합니다(보통 `<head>` 안).
...
- 두 줄 이상의 헤더를 만들거나 헤더에 배경색, 글자색 등 속성을 적용하려면 [(col)Header](/docs/props/col/header) 속성을 참고하세요.
...
이때 `el`이 가리키는 `div`가 **화면(DOM)에 이미 존재해야** 하므로, `div`가 로드된 뒤에 `create`를 호출해야 합니다.  
...
시트에 데이터를 채우는 표준 방법은 **조회 함수**입니다. 데이터를 어디서 가져오는지에 따라 둘 중 하나를 씁니다.

- 이미 자바스크립트에서 가지고 있는 **JSON 데이터**를 넣을 때는 [loadSearchData()](/docs/funcs/core/load-search-data)를 사용합니다.
- **서버 URL을 호출**해 응답 데이터를 받아 넣을 때는 [doSearch()](/docs/funcs/core/do-search)를 사용합니다.
...
조회 함수가 받는 JSON 데이터의 형식은 [조회 응답 규격](/docs/dataStructure/data-structure)을 참고하세요.
...
추출 결과의 각 행에는 `STATUS`가 붙습니다. 이 값은 서버가 **입력(`Added`) / 수정(`Changed`) / 삭제(`Deleted`)** 를 구분하는 데 사용됩니다.
...
수정 여부는 [hasChangedData()](/docs/funcs/core/has-changed-data)로 확인하고, 서버로 저장하고 반영까지 한 번에 처리하려면 [doSave()](/docs/funcs/core/do-save)를 사용합니다.

`doSave()` 사용 시 서버가 돌려줘야 하는 응답 형식은 [저장 응답 규격](/docs/dataStructure/saving-structure)을 참고하세요.

## STEP 7. 시트가 안 보일 때 점검

- 개발자도구(F12)를 열고 네트워크 탭에서 STEP 1에서 추가한 파일들이 모두 **200**으로 로드됐는지 확인합니다.
...
- 메소드 호출 기초(파라미터 넘기는 법)는 [method 사용법 기초](/docs/funcs/method)
