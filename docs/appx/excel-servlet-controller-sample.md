# 엑셀 서버모듈 컨트롤러 전체 예제 ***(appendix)***

<!-- synonyms: 엑셀 서버모듈 컨트롤러, ibsheetFileImportExportController, 서블릿 엑셀 다운로드 업로드, jsp 대신 컨트롤러, down2Excel 컨트롤러, loadExcel 컨트롤러, directDown2Excel 컨트롤러, directLoadExcel 컨트롤러, down2Pdf, down2Text, loadText, spring 엑셀 컨트롤러, 복붙 컨트롤러 -->

> 제공되는 서버모듈 jsp(`Down2Excel.jsp`, `LoadExcel.jsp` 등)의 로직을 스프링 컨트롤러(서블릿)로 옮긴 **전체 예제**입니다.  
> 필요한 메서드를 복사해 쓰고, `사용자 환경 설정 #N` 주석 옵션은 필요한 것만 주석 해제해 사용합니다.  
> 이 방식을 언제 쓰는지(JSP를 쓸 수 없는 환경, 멀티파트 본문 선소비 등)와 짧은 설명은 [엑셀 서버 모듈 트러블슈팅](/docs/appx/excel-server-troubleshooting)을 참고하세요.
