...
- 시트객체의 높이가 정적인 행(헤더,합계 행)조차 표현하지 못할 정도로 작은 경우에는 시트가 정상적으로 표시되지 않을 수 있습니다.
- el 크기는 **인라인 `style`로** 지정해야 시트에 반영됩니다. CSS 클래스나 선택자(예: `#id{}`)로 준 크기는 기본적으로 무시됩니다.
- CSS class로 지정한 크기를 쓰려면 [UseClassStyle](/docs/props/cfg/use_class-style)를 `1`로 설정합니다.
...
화면 전체를 시트로 채우고 브라우저 크기에 따라 자동으로 늘이고 줄이려면, 높이를 최상위(`html` / `body`)부터 시트까지 `100%`로 끊김 없이 연결합니다. (페이지 일부 영역에만 둘 때는 위 **2번**처럼 부모 DIV에 크기를 지정하세요.)
...
- 컬럼 너비도 영역을 채우려면 [(Col)RelWidth](/docs/props/col/rel-width)(또는 `Def.Col.RelWidth:1`)을 사용합니다.
- 높이 사슬이 한 곳이라도 끊기면(`0`이면) 시트가 보이지 않습니다. 너무 작아질 때를 대비해 `html, body`나 `.page`에 `min-width` / `min-height`로 최소 크기를 둘 수 있습니다.
