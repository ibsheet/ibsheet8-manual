...
> 이 함수는 호출할 때마다 지정한 내용으로 `FormulaRow` 설정 전체를 다시 지정합니다. 그래서 일부 컬럼만 지정하거나 감추기용으로 `visible`만 지정해 호출하면, 컬럼에 설정했던 기존 합계 설정이 사라집니다. 기존 합계를 유지하려면 호출 시 기존 컬럼 설정까지 함께 지정하세요.
